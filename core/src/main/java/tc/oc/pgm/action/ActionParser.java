...
public class ActionParser {
  ...
  public <T extends Filterable<?>> SummonAction<T> parseSummon(Element el, Class<T> scope)
    ...
    var entityId = parser.reference(TaggedMob.class, el, "entity-id").optional().orElse(null);
    Element mobEl = el.getChild("mob");

    if (entityId != null && mobEl != null) {
      throw new InvalidXMLException("Cannot specify both 'entity-id' and 'mob' elements", el);
    }

    if (entityId == null && mobEl == null) {
      throw new InvalidXMLException("Either 'entity-id' or a 'mob' element is required", el);
    }
    ...
    return new SummonAction<>(
        scope, entityId, mob, xFormula, yFormula, zFormula, pitchFormula, yawFormula);
  }
}
